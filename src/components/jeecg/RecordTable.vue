    ...
    <a-row type="flex">
      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button v-if="buttonPermission('add')" type="primary" icon="plus" @click="handleClickAdd" :disabled="disabled">添加</a-button>
          <span class="gap"></span>
            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              >
                ...
                <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">
                  ...
                  <a-radio-group v-model="radioId" @change="onChange">
                    <template v-for="(id,i) in [`${row.id}`]">
                        <a-radio
                        :id="id" :value="id"
                        :key="i" :disabled="!handleReadOnly(row)"
                    ...
                    <label :key="i" v-if="col.type === formTypes.input || col.type === formTypes.inputNumber">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <input
                          ...
                          :id="id" :disabled="handleReadOnly(row,col)"
                ...
                <a v-if="isSaveBtn&&(!handleReadOnly(row))" class="td"   :disabled="disabled" @click="() => handleSaveItemRow(row,rows)">保存</a>
                <a v-if="isSaveBtn&&(handleReadOnly(row))" class="td"  :disabled="disabled" @click="() => handleEditItemRow(row,rows)">编辑</a>
              </div>
              <!-- -- tr end -- -->
    ...
    methods: {
      ...
      handleReadOnly(row){
        ...
        if(this.isDisId.indexOf(newId)>-1){
            ...
            // console.log(this.isDisId.indexOf(newId)>-1,'this.isDisId.indexOf(newId)>-1---------------')
            return false
          }
        // if(localStorage.getItem("flag") == "add"){
          
        // }
        // else if(localStorage.getItem("flag") == "edit"){
        //   if(this.editFlag){
        //     return true
        //   }else{
        //      if(this.isDisId.indexOf(newId)>-1){
      ...
      /** 删除被选中的行 */
      removeSelectedRows() {
        this.$emit('removeSelectedRows', this.selectedRowIds[0])//把要删除的id传给父组件，因为是radio，所以每次就一个id
      ...
      handleChangeLeftCheckbox(event) {
        ...
        // }
        this.$emit('handleChoiceItemRow',id)
      },
      handleClickAdd() {
        this.add()
      },
