    ...
    <a-row type="flex">
      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button type="primary" icon="plus" v-if="isUserInfo" @click="handleAddUserDepart" :disabled="disabled">添加</a-button>
          <span class="gap"></span> 
              ...
              >
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <!-- date -->
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime || col.type === 'ymdate'|| col.type === 'yeartime'">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-date
                          v-if="isEditRow(row, col)"
                          :id="id"
                          :key="i" :disabled="handleReadOnly(row,col)"
                          v-bind="buildProps(row,col)"
                          style="width: 100%;"
                          ...
                          :dateFormat="customdate(col.type)"
                          allowClear
                 ...
                 <!-- <a v-if="isSaveBtn" :disabled="disabled" class="td"  @click="() => handleSavePeopleItemRow(row,rows)">保存</a> -->
                 <!-- <a v-if="isSaveBtn&&(handleReadOnly(row))" :disabled="disabled" class="td"  @click="() => handleEditPeopleItemRow(row,rows)">编辑</a> -->
              </div>
              <!-- -- tr end -- -->
...
<script>
  ...
  export default {
      ...
      dataSource: {
        ...
        handler: function (newValue) {
          ...
          this.getElementPromise('tbody').then(() => {
            ...
            if(localStorage.getItem("flag") == "edit" && this.addCreated==1){
              for (let index = 0; index < this.dataSource.length; index++) {
                this.handleSaveItemRow2(this.dataSource[index])
              }
            }
          })
        }
    ...
    methods: {
      //判断日期格式是否为年月
      customdate(coltype){
        // console.log(11,coltype)
        if (coltype=='ymdate'){
          return 'YYYY-MM'
        }else if(coltype=='datetime') {
          return 'YYYY-MM-DD HH:mm:ss'
        }else if(coltype=='yeartime') {
          return 'YYYY'
        }else{
          return 'YYYY-MM-DD'
        }
      ...
      //每一行保存
      handleSavePeopleItemRow(row,rows){
        // 获取整行的数据
        let { values } = this.getValuesSync({ validate: false, rowIds: [row.id] })
        let newId = this.handleId(row.id)
        let a = this.isDisId.filter(item=>item != newId)
        a.push(newId)
        this.isDisId = a
        this.editFlag = false
        console.log('保存该行数据',values[0])
        this.$emit('handleSavePeopleItemRow',true,values[0])
      },
      //每一行保存2
      handleSaveItemRow2(row){ 
        // 获取整行的数据
        let a = this.isDisId.filter(item=>item != row.id)
        // console.log('this.isDisId222',a)
        a.push(row.id)
        this.isDisId = a
        this.editFlag = false
      },
      //每一行编辑
      handleEditPeopleItemRow(row){
        this.editFlag = false
        let newId = this.handleId(row.id)
        this.isDisId = this.isDisId.filter(item=>item != newId)
      },
      //判断新增编辑详情的时候的保存编辑是否可用
      handleReadOnly(row){
        // let f = this.dataSource.filter(i=> i.id)
        // console.log(f,'row,col----------row,col')
        let newId = this.handleId(row.id)
        if(this.isDisId.indexOf(newId)>-1){
            return true
        }else if(localStorage.getItem("flag") == "detail"){
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          this.columns.forEach(column => {
            ...
            } else if (column.type === FormTypes.date || column.type === FormTypes.datetime || column.type === 'ymdate'|| column.type === 'yeartime') {
      ...
      /** 删除被选中的行 */
      removeSelectedRows() {
        this.$emit('removePeopleSelectedRows', this.selectedRowIds)
        console.log(this.selectedRowIds,'this.selectedRowIds参与人员选中删除')
      ...
      handleInputCommono(target, index, row, column) {
        // console.log(target, index, row, column,'target, index, row, column------------input')
