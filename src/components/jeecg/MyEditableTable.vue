              ...
              >
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <!-- date -->
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime || col.type === 'ymdate'">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-date
                          v-if="isEditRow(row, col)"
                          :id="id"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          this.columns.forEach(column => {
            ...
            } else if (column.type === FormTypes.date || column.type === FormTypes.datetime || column.type === 'ymdate') {
      ...
      //获取没有授权的按钮编码
      loadExcludeCode(){
        // console.log(this.authPre,'------------------this.authPre---------')
      ...
      buttonPermission(code){
        console.log(code,'---------code-----------',this.excludeCode.indexOf(code)<0)
