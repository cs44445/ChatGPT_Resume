            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              >
                ...
                <div
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="i">
                      ...
                      <div :hidden="uploadValues[id] != null">
                        <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                          <a-upload
                            name="file"
                            :data="{'isup':1}" :disabled="handleReadOnly(row,col)"
                            :multiple="false"
                 ...
                 <a v-if="isSaveBtn&&(!handleReadOnly(row))" class="td" :disabled="disabled" @click="() => handleSaveFileItemRow(row,rows)">保存</a>
                 <a v-if="isSaveBtn&&(handleReadOnly(row))" class="td" :disabled="disabled"  @click="() => handleEditFileItemRow(row,rows)">编辑</a>
              </div>
              <!-- -- tr end -- -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //每一行保存
      handleSaveFileItemRow(row,rows){
        // 获取整行的数据
        ...
        values[0].createBy = ""
        values[0].createTime = ""
        this.$emit('handleSaveFileItemRow',true,values[0])
      },
      //每一行编辑
      handleEditFileItemRow(row){
        this.editFlag = false
        let newId = this.handleId(row.id)
      ...
      /** 删除被选中的行 */
      removeSelectedRows() {
        this.$emit('removeFileSelectedRows', this.selectedRowIds)
        console.log(this.selectedRowIds,'this.selectedRowIds附件选中删除')
