<template>
  <a-card :bordered="false" style="background: #F7F8FA;">
    ...
    <div>
      ...
      <a-table :columns="columns" :scroll="{ x: 1500 }" size="small" :pagination="false" :dataSource="dataSource"
        :loading="loading" :expandedRowKeys="expandedRowKeys" @expandedRowsChange="handleExpandedRowsChange">
        <!-- :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" -->
        <span slot="action" slot-scope="text, record" class="fix-action">
          <span @click="handleEdit(record)"><a-icon type="edit" class="activeLogoCss" /></span>
          <a-dropdown>
