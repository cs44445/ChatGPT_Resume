<template>
    <div class="index-tab">
      <a-tabs default-active-key="2" :active-key="tabKey" @change="callback" class="index-tab" :tabBarGutter="5">
        <a-tab-pane key="1" disabled>
          <template #tab><span class="first-tab">付款报告</span></template>
        </a-tab-pane>
        <a-tab-pane key="2" tab="付款报告" force-render>            
           <BusPaymentReportList></BusPaymentReportList>
        </a-tab-pane>
      </a-tabs>
    </div>
  </template>
  ...
  <script>
    ...
    import BusPaymentReportList from './modules/BusPaymentReportList.vue'
    export default {
      name: 'BusTeamsList',
      mixins:[mixinDevice],
      components: {
        BusPaymentReportList
      },
      data () {
        return {
          description: '团队表管理页面',
