<template>
  <a-card :bordered="false" style="background: #F7F8FA;">
    ...
    <div>
      ...
      <a-table ref="table" size="small" :scroll="{ x: true }" rowKey="id" :columns="columns" :dataSource="dataSource"
        ...
        <span slot="action" slot-scope="text, record" class="fix-action">
          <!-- 1通过 2 驳回 -->
          <span @click="handleCheck(record)" v-if="record.formUrl == 'supplierWorkerApply:check'" class="activeLogoCss">审核</span>
          <span @click="handleCheck(record)" v-else-if="record.formUrl == 'supplierWorkerApply:resubmit'" class="activeLogoCss">办理</span>
    ...
    <supplier-worker-apply-modal ref="supplierWorkerForm" :busApplyAuditForm="busApplyAuditForm" @handlePass="handlePass" />

    <!-- 作者签入驳回重新提交 -->
    <supplier-worker-reject-modal ref="supplierWorkerRejectForm" :busApplyAuditForm="busApplyAuditForm"
...
import SupplierWorkerApplyModal from './modules/SupplierWorkerApply/SupplierWorkerApplyModal'
import SupplierWorkerRejectModal from './modules/SupplierWorkerApply/SupplierWorkerRejectModal'
...
export default {
  name: 'BusApplyWorkerList',
  mixins: [JeecgListMixin, mixinDevice],
  components: {
    SupplierWorkerApplyModal,
    SupplierWorkerRejectModal,
    BusApprHistoryModal
  ...
  methods: {
    ...
    handleCheck(record) {
      ...
      if (record.formUrl == 'supplierWorkerApply:check') {
        this.$refs['supplierWorkerForm'].visible = true
        this.$refs['supplierWorkerForm'].title = '审核'
      } else if (record.formUrl == 'supplierWorkerApply:resubmit') {
        this.$refs['supplierWorkerRejectForm'].visible = true
        this.$refs['supplierWorkerRejectForm'].title = '办理'
      } 
