<template>
  <j-modal :title="title" :width="width" :visible="visible" :closable="false"
            ...
            <div class="steps-content">
              ...
              <a-form-model ref="stepform" :model="model" slot="detail" :rules="validatorRules">
                <div class="steps-right-btn">
                  <a-button href="javascript:;" style="color: #ffffff;margin-left: 5px;background: #cc148a;height: 24px;line-height: 22px;" icon="save"
                    @click="saveStep">保存</a-button>
                  <a-button href="javascript:;" style="color: #2f3133;margin-left: 5px;height: 24px;line-height: 22px;" @click="resetForm"><img
                      src="~@/assets/clear.svg" width="18px" style="margin-bottom:5px;">&nbsp;清空</a-button>
                  <a-button href="javascript:;" style="margin-left: 5px;height: 24px;line-height: 24px;" v-show="stepsInit.length != 1"
                    @click="delStep"><img src="~@/assets/delete.svg" width="20px"
                      style="color: #2f3133;margin-bottom:5px;">&nbsp;删除</a-button>

                  <a-button v-if="stepsInit.length > 1 && current > 0" style="color: #2f3133;margin-left: 5px;height: 24px;line-height: 22px;"
                    icon="arrow-up" @click="prev">向上移动</a-button>
                  <a-button v-if="stepsInit.length > 1 && stepsInit.length - 1 != current"
                    style="color: #2f3133;margin-left: 5px;height: 24px;line-height: 22px;" icon="arrow-down" @click="next">向下移动</a-button>
                </div>
                <a-row style="height:500px;overflow-y:scroll;">
                  <a-col :span="24" style="position:relative;padding: 0 20px;" class="no-line-height">
                    <a-form-model-item label="里程碑" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="milepostCode"
                      ...
                      <a-auto-complete v-model="model.milepostCode" :data-source="selectDictOptions" placeholder="请选择里程碑"
                        @change="onSelectChange" />
                    </a-form-model-item>
                    <a-tooltip style="position:absolute;top:0px;left:75px;">
                      <template slot="title">
                        自定义里程碑名称可手动输入
                      </template>
                    ...
                    <a-form-model-item label="负责人" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="owner"
                      colon="false" style="margin-bottom:12px;">
                      <a-select placeholder="请选择负责人" v-model="model.owner" @change="changeLeader">
                        <a-select-option :value="item.userId" v-for="item in leaderList" :key="item.userId">
                          {{ item.userName }}
                        </a-select-option>
                      </a-select>
                    </a-form-model-item>
                    <a-form-model-item label="周期" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="beginTime"
                      colon="false" style="margin-bottom:12px;">
                      <a-date-picker v-model="model.beginTime" :disabled-date="disabledStartDate" valueFormat="YYYY-MM-DD"
                        placeholder="Start" @openChange="handleStartOpenChange" style="width:211px;" />
                      &nbsp;至&nbsp;
                      <a-date-picker v-model="model.endTime" :disabled-date="disabledEndDate" valueFormat="YYYY-MM-DD"
                        placeholder="End" :open="endOpen" @openChange="handleEndOpenChange" style="width:211px;" />
                    </a-form-model-item>
                  </a-col>
                  <a-row style="padding: 0 20px;" class="no-line-height">
                    <a-col :span="6">
                      <a-form-model-item label="预估采购成本" :labelCol="labelCol" :wrapperCol="wrapperCol" colon="false"
                        prop="estimatedProcurementCost" style="margin:0 10px 12px 0;">
                        <a-input-number style="width: 100%" v-model="model.estimatedProcurementCost"
                          placeholder="请输入预估采购成本" :min="0"></a-input-number>
                      </a-form-model-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-model-item label="预估差旅费用" :labelCol="labelCol" :wrapperCol="wrapperCol" colon="false"
                        prop="estimatedTravelCost" style="margin:0 10px 12px 0;">
                        <a-input-number style="width: 100%" v-model="model.estimatedTravelCost" placeholder="请输入预估差旅费用"
                          :min="0"></a-input-number>
                      </a-form-model-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-model-item label="预估分包费用" :labelCol="labelCol" :wrapperCol="wrapperCol" colon="false"
                        prop="estimatedSubcontractingCost" style="margin:0 10px 12px 0;">
                        <a-input-number style="width: 100%" v-model="model.estimatedSubcontractingCost"
                          placeholder="请输入预估分包费用" :min="0"></a-input-number>
                      </a-form-model-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-model-item label="预估其他费用" :labelCol="labelCol" :wrapperCol="wrapperCol" colon="false"
                        prop="estimatedOtherCost" style="margin:0 0px 12px 0;">
                    ...
                    <a-form-model-item v-for="(domain, index) in model.milepostLaborList" :key="domain.key"
                      ...
                      <div style="background: #ebebed;">
                        <div style="display:flex;justify-content: space-between;width: 100%;">
                          <a-form-model-item label="角色" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }" colon="false"
                            ...
                            <a-select v-model="domain.costRoleId" placeholder="请选择角色" :allowClear="true">
                          ...
                          <a-form-model-item label="预估工时(h)" :labelCol="{ span: 14 }" :wrapperCol="{ span: 10 }"
                            colon="false" style="width:45%;margin-bottom:0;">
                            <a-input-number :min="0" v-model.lazy="domain.estimatedHours" style="width:100%;" />
                          </a-form-model-item>
                          <a-form-model-item style="width:5%;margin-bottom:0;margin-right: 5px;">
                      ...
                      <a-form-model-item style="width:25px;margin-bottom:0;">
                        <div @click="addDomain" v-if="model.milepostLaborList.length - 1 == index">
                          <div class="addsty-cust">
                            <a-icon slot="icon" type="plus" style="font-size: 16px; color: #2F3133" />
                          </div>
                        </div>
                      </a-form-model-item>
    ...
    <template slot="footer">
      <!-- <a-button v-if="stepsInit.length > 1 && current != 0" style="margin-left: 8px" @click="prev"> 向上移动 </a-button>
      <a-button v-if="stepsInit.length > 1 && stepsInit.length - 1 != current" @click="next"> 向下移动 </a-button> -->
