<template>
  <j-modal :title="title" :width="width" :visible="visible" switchFullscreen @cancel="handleCancel"
    ...
    <div>
      <h4 style="margin:0 0 20px 0;">计划工时（H）： {{ planHours }} <span style="margin-left:20px;">投入工时（H）： {{
        actualHours
      }}</span>
    ...
    <a-table size="small" ref="table" :scroll="{ y: 220 }" rowKey="id" :columns="columns2" :dataSource="dataSource"
      :pagination="false" :rowSelection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" class="j-table-force-nowrap">
